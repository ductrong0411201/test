...
class QueryBuilderRequest extends Request
{
    ...
    public function includes(): Collection
    {
        ...
        if (!is_array($includeParts)) {
            $includeParts = explode(config('query-builder.delimiter.include', ','), $includeParts);
        }
    ...
    public function pagination(): array
    {
        ...
        $page = $this->get($fieldsPageName, 1);
        $per_page = $this->get($fieldsPerPageName, config('query-builder.pagination.default_size'));
        $is_paginate = $this->get($fieldsIsPaginateName, config('query-builder.pagination.is_paginate'), true);
